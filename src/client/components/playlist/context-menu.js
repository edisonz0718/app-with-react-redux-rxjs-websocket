...
export class PlaylistContextMenuComponent extends ElementComponent {
    ...
    _onAttach () {
        ...
        const openMenuOnItem$ = Observable.fromEventNoDefault(this._$list, "contextmenu") //jquery contextmenu event handler
            .map(event=> $(event.target).closest("li").data("component"));//get playlist item component
        const closeMenu$ = Observable.fromEvent($("body"),"mouseup")
            .filter(event => $(event.target).closest("li.selected, .context-menu").length == 0)
            .mapTo(null);// close menu only when it's not descendent of li.selected and context-menu
        ...
        Observable.merge(setCurrentItem$, deleteItem$)
            ...
            .mergeMap(([op, item]) => op(item).catchWrap())
            .compSubscribe(this, response => {
                if(response && response.error)
                    alert(response.error.message || "Unknown Error");
                else
                    selectedItemSubject$.next(null);// Used to close context menu after click play or delete in context menu
            });
    }
}